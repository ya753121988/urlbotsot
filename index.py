...
stats_col = db['stats'] # অ্যানালিটিক্স ডাটার জন্য
...
def get_channels_html(theme_color="sky"):
    ...
    html = f'<div class="w-full max-w-5xl mx-auto mt-12 mb-8 p-8 rounded-[40px] border-2 border-white/10 glass shadow-2xl"><h3 class="text-center {c["text"]} font-black mb-10 uppercase tracking-[0.3em] text-lg">Partner Channels</h3><div class="flex flex-col items-center gap-10">'
    for ch in channels:
        html += f'<a href="{ch["link"]}" target="_blank" class="flex flex-col items-center gap-3 group transition-transform hover:scale-105"><div class="text-center"><p class="text-lg font-black text-gray-100 uppercase italic tracking-wider">{ch.get("name", "Join Our Channel")}</p></div><img src="{ch["logo"]}" style="width: 320px; height: 180px;" class="object-cover border-2 border-white/10 rounded-lg group-hover:border-white/40 shadow-2xl transition"></a>'
    return html + '</div></div>'
...
def api_system():
    ...
    shortened_url = request.host_url + sc
...
def admin_panel():
    ...
    <!DOCTYPE html><html><head><script src="https://cdn.tailwindcss.com"></script><script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
    <style> .tab-content { display: none; } .tab-content.active { display: block; } .active-btn { background: #1e293b !important; color: white !important; } </style>
    </head><body class="bg-slate-50 flex min-h-screen font-sans">
        <div class="w-72 bg-white border-r p-8 flex flex-col">
            <h2 class="text-2xl font-black mb-12 text-blue-600 italic">PREMIUM ADMIN</h2>
            <nav class="space-y-2 flex-1">
                <button onclick="tab('dash')" id="btn-dash" class="w-full text-left p-4 rounded-xl font-bold active-btn">📊 Dashboard</button>
                <button onclick="tab('links')" id="btn-links" class="w-full text-left p-4 rounded-xl font-bold text-slate-500">🔗 Links List</button>
                <button onclick="tab('config')" id="btn-config" class="w-full text-left p-4 rounded-xl font-bold text-slate-500">⚙️ Settings</button>
                <button onclick="tab('ads')" id="btn-ads" class="w-full text-left p-4 rounded-xl font-bold text-slate-500">💰 Ads Manage</button>
                <button onclick="tab('partners')" id="btn-partners" class="w-full text-left p-4 rounded-xl font-bold text-slate-500">📢 Partners</button>
            </nav>
            <a href="/logout" class="mt-10 p-4 bg-red-50 text-red-600 rounded-xl text-center font-bold">LOGOUT</a>
        </div>
        <div class="flex-1 p-12 overflow-y-auto">
            <div id="dash" class="tab-content active space-y-8">
                <div class="grid grid-cols-3 gap-6">
                    <div class="bg-blue-600 p-8 rounded-3xl text-white shadow-xl"><p class="text-xs font-bold opacity-70">LIFETIME VIEWS</p><h3 class="text-5xl font-black">{{total_views}}</h3></div>
                    <div class="bg-emerald-500 p-8 rounded-3xl text-white shadow-xl"><p class="text-xs font-bold opacity-70">TODAY'S VIEWS</p><h3 class="text-5xl font-black">{{today_views}}</h3></div>
                    <div class="bg-white p-8 rounded-3xl border shadow-sm"><p class="text-xs font-bold text-slate-400">TOTAL LINKS</p><h3 class="text-5xl font-black text-slate-800">{{all_urls|length}}</h3></div>
                </div>
                <div class="grid grid-cols-2 gap-8">
                    <div class="bg-white p-8 rounded-3xl border"><h4 class="font-black mb-6">Traffic Trend</h4><canvas id="trafficChart"></canvas></div>
                    <div class="bg-white p-8 rounded-3xl border">
                        <h4 class="font-black mb-6">Top Countries</h4>
                        {% for c in countries %}<div class="flex justify-between p-3 bg-slate-50 mb-2 rounded-xl"><span>{{c._id}}</span><b>{{c.count}}</b></div>{% endfor %}
                        <h4 class="font-black mt-8 mb-4">Devices</h4>
                        {% for d in devices %}<div class="flex justify-between p-3 bg-blue-50 mb-2 rounded-xl"><span>{{d._id}}</span><b>{{d.count}}</b></div>{% endfor %}
                    </div>
                </div>
                <div class="bg-white p-8 rounded-3xl border"><h4 class="font-black mb-4">Direct Ads Tracking</h4>
                    <div class="space-y-2">{% for as in ad_stats %}<div class="flex justify-between p-4 bg-slate-50 rounded-xl"><span class="truncate max-w-md">{{as.url}}</span><b class="text-emerald-600">{{as.count}} clicks</b></div>{% endfor %}</div>
                </div>
            </div>
            <div id="links" class="tab-content space-y-6">
                <div class="bg-white rounded-3xl border overflow-hidden">
                    <table class="w-full text-left">
                        <thead class="bg-slate-50 text-xs font-bold"><tr><th class="p-6">Time</th><th class="p-6">Code</th><th class="p-6">URL</th><th class="p-6">Clicks</th></tr></thead>
                        <tbody class="divide-y">{% for u in all_urls %}<tr><td class="p-6 text-xs">{{u.created_at}}</td><td class="p-6 font-bold text-blue-600">/{{u.short_code}}</td><td class="p-6 truncate max-w-xs">{{u.long_url}}</td><td class="p-6">{{u.clicks}}</td></tr>{% endfor %}</tbody>
                    </table>
                </div>
            </div>
            <div id="config" class="tab-content">
                <form action="/admin/update" method="POST" class="bg-white p-10 rounded-3xl border space-y-6">
                    <div class="grid grid-cols-2 gap-6">
                        <input type="text" name="site_name" value="{{s.site_name}}" class="p-4 bg-slate-100 rounded-xl font-bold" placeholder="Site Name">
                        <input type="text" name="admin_telegram_id" value="{{s.admin_telegram_id}}" class="p-4 bg-slate-100 rounded-xl font-bold" placeholder="Telegram Admin ID">
                        <input type="number" name="steps" value="{{s.steps}}" class="p-4 bg-slate-100 rounded-xl font-bold" placeholder="Steps">
                        <input type="number" name="timer_seconds" value="{{s.timer_seconds}}" class="p-4 bg-slate-100 rounded-xl font-bold" placeholder="Timer">
                        <select name="main_theme" class="p-4 bg-slate-100 rounded-xl">{% for k in colors %}<option value="{{k}}" {% if s.main_theme == k %}selected{% endif %}>HOME: {{k|upper}}</option>{% endfor %}</select>
                        <select name="step_theme" class="p-4 bg-slate-100 rounded-xl">{% for k in colors %}<option value="{{k}}" {% if s.step_theme == k %}selected{% endif %}>STEP: {{k|upper}}</option>{% endfor %}</select>
                        <input type="number" name="direct_click_limit" value="{{s.direct_click_limit}}" class="p-4 bg-blue-50 rounded-xl font-bold" placeholder="Ad Limit">
                        <input type="password" name="new_password" class="p-4 bg-red-50 rounded-xl font-bold" placeholder="New Admin Pass">
                    </div>
                    <div class="grid grid-cols-2 gap-6">
                        <textarea name="popunder" placeholder="Popunder Script" class="h-24 p-4 bg-slate-100 rounded-xl text-xs font-mono">{{s.popunder}}</textarea>
                        <textarea name="banner" placeholder="Banner Script" class="h-24 p-4 bg-slate-100 rounded-xl text-xs font-mono">{{s.banner}}</textarea>
                    </div>
                    <button class="w-full bg-slate-900 text-white py-6 rounded-2xl font-black text-xl">SAVE ALL CHANGES</button>
                </form>
            </div>
            <div id="ads" class="tab-content space-y-8">
                <div class="bg-white p-10 rounded-3xl border">
                    ...
                    <form action="/admin/add_ad_link" method="POST" class="flex gap-4 mb-8">
                        <input type="url" name="ad_url" placeholder="Ad Link URL" class="flex-1 p-4 bg-slate-100 rounded-xl" required>
                        <button class="bg-emerald-600 text-white px-10 rounded-xl font-bold">ADD LINK</button>
                    </form>
                    {% for l in ad_links %}<div class="flex justify-between p-4 bg-slate-50 mb-2 rounded-xl"><span>{{l.url}}</span><a href="/admin/delete_ad_link/{{l._id}}" class="text-red-500 font-bold">Delete</a></div>{% endfor %}
                </div>
            </div>
            <div id="partners" class="tab-content">
                <div class="bg-white p-10 rounded-3xl border">
                    <form action="/admin/add_channel" method="POST" class="grid grid-cols-4 gap-4 mb-10">
                        <input type="text" name="name" placeholder="Name" class="p-4 bg-slate-100 rounded-xl" required>
                        <input type="url" name="logo" placeholder="Logo URL" class="p-4 bg-slate-100 rounded-xl" required>
                        <input type="url" name="link" placeholder="Invite Link" class="p-4 bg-slate-100 rounded-xl" required>
                        <button class="bg-blue-600 text-white rounded-xl font-bold">ADD CHANNEL</button>
                    </form>
                    {% for ch in channels %}<div class="flex items-center gap-6 p-4 border-b">
                        <img src="{{ch.logo}}" class="w-32 h-20 object-cover rounded-lg">
                        <div class="flex-1"><b>{{ch.name}}</b><br><small>{{ch.link}}</small></div>
                        <a href="/admin/delete_channel/{{ch._id}}" class="text-red-500 font-bold">Delete</a>
                    </div>{% endfor %}
                </div>
            </div>
        </div>
        <script>
            ...
            new Chart(document.getElementById('trafficChart'), {
                type: 'line',
                data: { labels: {{chart_labels|tojson}}, datasets: [{ label: 'Views', data: {{chart_values|tojson}}, borderColor: '#2563eb', fill: true, backgroundColor: 'rgba(37, 99, 235, 0.1)', tension: 0.4 }] },
...
def handle_ad_steps(short_code):
    ...
    {{ s.popunder|safe }}</head><body class="bg-slate-50 flex flex-col items-center p-6 min-h-screen">
        ...
        <div class="bg-white p-12 md:p-20 rounded-[70px] shadow-2xl text-center max-w-2xl w-full border-t-[16px] {{tc.border}}">
            <p class="text-xl font-black {{tc.text}} uppercase mb-4">Step {{step}} of {{total_steps}}</p>
            <div id="timer_box" class="text-7xl font-black {{tc.text}} mb-8 {{tc.light_bg}} w-40 h-40 flex items-center justify-center rounded-full mx-auto border-8 {{tc.border}} shadow-inner">{{timer}}</div>
...
def forgot_password():
    if request.method == 'POST':
        ...
        if tg_id == settings.get('admin_telegram_id'):
            ...
            requests.post(f"https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}/sendMessage", data={"chat_id": tg_id, "text": f"🛡️ SECURITY OTP: {otp}"})
...
def verify_otp():
    ...
    return render_template_string('<body style="background:#0f172a;display:flex;justify-content:center;align-items:center;height:100vh;"><form method="POST" style="background:white;padding:40px;border-radius:30px;width:320px;text-align:center;"><h2>Verify OTP</h2><input type="text" name="otp" placeholder="ENTER OTP" required style="width:100%;padding:15px;margin:20px 0;text-align:center;font-size:24px;font-weight:bold;"><button style="width:100%;padding:15px;background:#10b981;color:white;border:none;border-radius:15px;font-weight:bold;">VERIFY</button></form></body>')
...
def reset_password():
    ...
    return render_template_string('<body style="background:#0f172a;display:flex;justify-content:center;align-items:center;height:100vh;"><form method="POST" style="background:white;padding:40px;border-radius:30px;width:320px;"><h2 style="text-align:center;">NEW PASSKEY</h2><input type="password" name="password" required placeholder="New Password" style="width:100%;padding:15px;margin:20px 0;"><button style="width:100%;padding:15px;background:#1e293b;color:white;border:none;border-radius:15px;font-weight:bold;">UPDATE</button></form></body>')
